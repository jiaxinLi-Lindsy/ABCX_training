...
function generateCSVContent(trialRecords, results, experimentInfo) {
    ...
    trialRecords.forEach(function(trial) {
        const row = [
            ...
            trial.foilLevel || 'N/A',
            trial.levelToFoilDistance || 'N/A',
            trial.modulationWindowPercent || 'N/A',
            trial.modulationWindowDuration || 'N/A',
            trial.foilWindowPercent || 'N/A',
            trial.foilWindowDuration || 'N/A',
            trial.modulationRate || 'N/A',
            trial.slope || 'N/A',
            trial.xMatchesType || 'N/A',
            trial.standardPosition || 'N/A',
            trial.levelPosition || 'N/A',
            trial.foilPosition || 'N/A',
            trial.userResponse,
...
function sendExperimentResults(params) {
    ...
    // 生成文件名格式：ID_SessionNumber_ExperimentName.csv
    // 从实验类型中提取简化名称
    let experimentName = experimentInfo.type || 'Experiment';
    
    // 根据实验类型简化文件名
    if (experimentName.includes('Level Tone')) {
        experimentName = 'Level_Tone_ABCX';
    } else if (experimentName.includes('Rising')) {
        experimentName = 'Rising_ABCX';
    } else if (experimentName.includes('Falling')) {
        experimentName = 'Falling_ABCX';
    }
    
    const fileName = `${experimentInfo.participantId}_${experimentInfo.sessionNumber}_${experimentName}.csv`;
